...
// define global int size of frequency vector
const int FRE_SIZE = 256;
...
int main(int argc, char* argv[]){
    ...
    for(unsigned int i =0; i < freqs.size();i++){
        if(freqs[i]!=0){
            cout<<(char)i<<(unsigned short)freqs[i]<<endl;
        }
        FOStrem.write<unsigned short>((unsigned short)freqs[i]);
    }
