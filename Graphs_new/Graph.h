...
class Graph {
private:
    unsigned int num_Edges; // Track number of edges

    // Nested unordered map to store all connected vertices of each vertex
    // and the weights between them.
    unordered_map<string, unordered_map<string,unsigned int>> edgesList;

    typedef pair<unsigned int,string> Gnode;

    // A look-up unordered map to store the mapping 
    // from labels to indices
    unordered_map<string, int> lbl2idx;

    // store symbol of each vertex in graph
    vector<string> vertices;

    void smt_recur(int & u, int & v, vector<bool> & done, 
...
public:
    ...
    /**
     * @brief Customized comparison class for nodefor Dijkstra's 
     * algorithm.
     */
    struct GnodeComp
    {
