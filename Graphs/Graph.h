 ...
 * 
 * This file is the skeleton of the graph object
 */
#ifndef GRAPH_H
#define GRAPH_H
...
private:
    ...
    // vector store all neighbor nodes and their weights
    // between the specific node in the order of vertices
    // vector
    vector<unordered_map<string, int>> edgeList;

    // typedef class to represent node for Dijkstra's
    // algorithm
    typedef pair<int,string> GNode;


    // vector to store symbol of nodes while maintaining
    // an order to build up edge list later
...
public:
     ...
     * we call this function on "A", we would return the following `vector`:
     * {"B", "C"}
     *
     * @param node_label The label of the query node.
    ...
    /**
     * Customized comparison class for nodefor Dijkstra's 
     * algorithm.
     */
    struct GNodeComp
    {
